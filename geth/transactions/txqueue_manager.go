...
func New(rpcClientProvider RPCClientProvider) *Manager {
	return &Manager{
		rpcClientProvider: rpcClientProvider,
		txQueue:           newQueue(),
		addrLock:          &AddrLocker{},
...
func (m *Manager) DiscardTransactions(ids []string) map[string]RawDiscardResult {
	results := make(map[string]RawDiscardResult)

	for _, txID := range ids {
		err := m.DiscardTransaction(txID)
		if err != nil {
			results[txID] = RawDiscardResult{
				Error: err,
			}
		}
	}
